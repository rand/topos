...
### 0. Task Tracking (bd Integration)
**Goal:** Mirror Conductor tasks in the `bd` task tracker for external visibility.
1.  **Initialization:** Ensure `bd` is initialized in the project root. Run `bd init` if the `.bd` directory is missing.
2.  **Creation:** Before starting a task in Conductor:
    -   Create a corresponding task in `bd`: `bd task add "<Task Name/Description>"`.
    -   Note the `task_id` returned by `bd`.
3.  **Completion:** After completing the task in Conductor (and verifying it):
    -   Mark the task as done in `bd`: `bd task done <task_id>`.
...
2.  **Verify bd Sync:** Confirm that all `bd` tasks associated with this phase are closed (`bd task list` should not show them).
